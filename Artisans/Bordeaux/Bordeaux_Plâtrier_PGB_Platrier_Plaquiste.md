---
nom: "PGB Plâtrier Plaquiste"
type: "Artisan"
metier: "Plâtrier"
zone: "Bordeaux"
statut: "à_contacter"
priorite: "moyenne"
site_actuel: ""
techno: "N/A"
date_contact: ""
date_reponse: ""
canal: ""
reponse: ""
telephone: "07 49 84 33 82"
email: ""
tags:
  - "prospect"
  - "sans-site"
  - "plâtrier"
---

# PGB Plâtrier Plaquiste

## 📋 Infos générales
- **Métier** : Plâtrier
- **Zone** : Bordeaux
- **Adresse** : Bordeaux
- **Téléphone** : 07 49 84 33 82
- **Email** : —
- **Site actuel** : —

## 🌐 Audit site web
- **A un site ?** : Non
- **Techno** : N/A
## ❌ Défauts identifiés (leviers de vente)
1. AUCUN site web = invisible sur Google
2. Perte de clients potentiels chaque jour
3. Pas de vitrine digitale pour présenter services et tarifs
4. Concurrence avec site web prend tous les appels

## 💰 Proposition de valeur
- Création site vitrine Next.js = visibilité immédiate
- SEO local optimisé = premiers résultats sur "plâtrier Bordeaux"
- Design professionnel = confiance et crédibilité
- Formulaire de contact = +40% de demandes

## 📝 Notes
- Plâtrier bien noté avec 17 avis Google mais aucun site web.
- Source : https://platrierinfo.com/ville/bordeaux_33000/
- Note Google : 4.5/5 (17 avis)

## 📧 Prospection
- **Date contact** : 
- **Canal** : Email / Téléphone
- **Réponse** : 
- **Suivi** : 
